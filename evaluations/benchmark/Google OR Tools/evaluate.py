...
if __name__ == '__main__':
    ...
    brandimarte_dataset = np.load(f'./brandimarte/Mk{instance}.fjs.npy')
    validation_dataset = np.load(f'./validation_set/MK{instance}_validation_set_4.npy')
    characteristic = characteristics[f'Mk{instance}.fjs']

    brandimarte = convert_numpy_dataset_to_jobs_array(brandimarte_dataset)
    validation = convert_numpy_dataset_to_jobs_array(validation_dataset[0])

    print('BRANDIMARTE')
    print(characteristic.num_of_machines)
    results = flexible_jobshop(brandimarte, characteristic.num_of_machines)

    # Determine the action sequences
    ...
    # print('VALIDATION')
    # flexible_jobshop(validation, characteristic.num_of_machines)
